...
def callset_vcf_path(build: int, data_type: str, data_source: str, version: int, sharded: bool) -> str:
    """
    Returns callset vcf path. Can be internal or external, exomes or genomes.
    """
    vcf_file_string = f"sharded_vcf/RDG_{data_type}_Broad_{data_source}.filtered.*.vcf.gz" if sharded else f"RDG_{data_type}_Broad_{data_source}.vcf.bgz"
