...
def prep_meta(ht: hl.Table) -> hl.Table:
    ...
    ht = ht.select('PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'data_type', 'qc_pop', 'source')
    ht = ht.filter(hl.is_defined(ht.PC1)).rename({'qc_pop': 'known_pop'})
    return ht


def annotate_all_sample_metadata(mt: hl.MatrixTable, build: int, data_type: str, data_source: str, version: int) -> hl.Table:
    """
...
def run_platform_imputation(mt: hl.MatrixTable, plat_min_cluster_size: int, plat_min_sample_size: int) -> hl.Table:
    ...
    d = {f'plat_PC{i}': scores_ht.scores[i] for i in list(range(1, 7))}
...
def run_population_pca(mt: hl.MatrixTable,
    ...
    d = {f'PC{i}': scores.scores[i] for i in list(range(1, 7))}
    ...
    data = meta.to_pandas()
    pc_cols = ['PC{}'.format(pc) for pc in list(range(1, 7))]
    pop_pca_ht, pop_clf = assign_population_pcs(data, pc_cols=pc_cols, fit=fit)
    ...
    pop_pca_ht = pop_pca_ht.filter(pop_pca_ht.source == "RDG").drop('known_pop', 'source',
                                                                    'data_type').rename({'pop': 'qc_pop'}).key_by('s')
    pop_pca_ht = pop_pca_ht.annotate(**scores[pop_pca_ht.s])
    pop_pca_ht = pop_pca_ht.rename({f'PC{pc+1}': f'pop_PC{pc+1}' for pc in range(6)})
    return pop_pca_ht
...
def main(args):
    ...
    #hl._set_flags(cpp='1')  # Use Hail's C++ where possible - speeds up reading and writing of MT and HT

    data_type = args.data_type
    build = args.build
    data_source = args.data_source
    version = args.callset_version
    plat_min_cluster_size = args.plat_min_cluster_size
    plat_min_sample_size = args.plat_min_sample_size
    fit = args.pop_pca_fit
    ...
    if is_test:
        ...
        mt = hl.filter_intervals(mt, [hl.parse_locus_interval('20', reference_genome=f'GRCh{build}')])
    ...
    meta_ht = annotate_all_sample_metadata(mt, build, data_type, data_source, version)
    ...
    if data_type == 'WES' and data_source == 'External':
        ...
        plat_ht = run_platform_imputation(mt, plat_min_cluster_size, plat_min_sample_size)
    ...
    # Checkpoint because mt.to_pandas in run_population_pca nukes nodes otherwise
    ...
    pop_ht = run_population_pca(mt, build, data_type, data_source, version, is_test, fit)
    ...
    mt.cols().flatten().export(ht_to_tsv_path(build, data_type, data_source, version, is_test))
