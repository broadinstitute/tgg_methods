# Seqr Code for JUST Pop Imputation, adapted from seqr_sample_qc.py
import argparse
import json
import logging
import pickle
...
def run_population_pca(
    ...
    :rtype: Table
    """
    if v2_cmg_model and v4_custom_model_path:
        raise ValueError("Cannot request two models")

    if v4_custom_model_path:
        logger.info(
            "Reading in custom RF model based off of gnomAD v4 from custom path..."
        )
        loadings = hl.read_table(rdg_gnomad_v4_pop_pca_loadings_path())
        model_path = v4_custom_model_path
    ...
    scores = (
        ...
        .key_by("s")
        .checkpoint(
            new_temp_file("scores_temp", extension="ht").replace(
...
def main(args):
    ...
    v4_custom_model_path = args.v4_custom_model_path
    v4_custom_per_pop_probs = args.v4_custom_per_pop_probs
    v4_custom_per_pop_probs_path = args.v4_custom_per_pop_probs_path
    ...
    def _get_root_sqc_output(
        ...
        version,
        data_source,
        v4_custom_model_path,
        v2_cmg_model,
        output_suffix=None,
    ) -> None:

        v4_custom_model = True if v4_custom_model_path else False

        model_text = "gnomAD_v4"
        if v4_custom_model:
            model_text = "v4_custom"
        elif v2_cmg_model:
    ...
    pop_ht = run_population_pca(
        ...
        v4_custom_model_path=v4_custom_model_path,
    )
    ...
    if v4_custom_per_pop_probs:
        logger.info("Assigning pops with per-pop probabilities...")
        custom_probs = "gs://marten-seqr-sandbox-storage/ancestry/rf_10162024/default_v4_pop_min_probs.json"
        if v4_custom_model_path:
            logger.info("Reading custom set of per pop probs...")
            custom_probs = v4_custom_per_pop_probs_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--v4-custom-model-path",
        help="Path to custom RF model, built off of gnomAD v4",
        type=str,
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--v4-custom-per-pop-probs-path",
        help="Assign imputed genetic ancestry using CUSTOM per pop probabilities",
        type=str,
        default=None,
    )
    parser.add_argument(
